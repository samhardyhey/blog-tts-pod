...
from config import coqui_vkts_speaker_indices, data_dir
...
def sample_tts_providers():
    ...
    #     save_path = output_dir / "coqui" / f"{to_snake_case(Path(coqui_model).name)}_nautilus_editors_note.mp3"
    #     tts_coqui_single_speaker(coqui_model, nautilus_editors_note, save_path)

    for speaker_index in coqui_vkts_speaker_indices:
        save_path = (
            output_dir / "coqui" / f"vtks_{speaker_index}_nautilus_editors_note.mp3"
        )
